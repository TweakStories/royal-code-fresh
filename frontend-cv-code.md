        ...
        <div>
          <royal-code-ui-title [level]="TitleTypeEnum.H3" [text]="'cv.contact.formTitle' | translate" extraClasses="!text-xl !font-semibold mb-4" />
           <form [formGroup]="contactForm" (ngSubmit)="onSubmit()" class="space-y-4">
            ...
            <input type="text" formControlName="verifyme" class="sr-only" aria-hidden="true" tabindex="-1" autocomplete="off">
  ...
  template: `
    ...
    <div id="home-main" class="container-max space-y-24 md:space-y-32 md:py-16">
      ...
      <section class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[70vh] relative">
        ...
        <section #animatedSection data-section-id="social-proof" class="fade-in-section text-center relative py-16 rounded-2xl">
           <div class="relative z-10">
             ...
             <div class="flex justify-center items-center gap-8 opacity-60 hover:opacity-100 transition-all duration-300">
                  <royal-code-ui-image src="images/brands/heineken-logo.webp" alt="Heineken logo" class="h-12" [rounding]="'none'" />
                  <royal-code-ui-image src="images/brands/fintech-logo.webp" alt="FinTech Scale-up logo" class="h-12" [rounding]="'none'" />
                  <royal-code-ui-image src="images/brands/crypto-guru-logo.webp" alt="Crypto Guru logo" class="h-12" [rounding]="'none'" />
             </div>
             @if (featuredTestimonial(); as testimonial) {
              <div class="max-w-2xl mx-auto mt-8">
                  <royal-code-ui-card borderColor="gradient" extraContentClasses="bg-card/80 backdrop-blur-lg grid grid-cols-[auto,1fr] items-center gap-4 text-left">
                      <royal-code-ui-image [image]="testimonial.authorImage" [fallbackSrc]="'images/users/cto-avatar.webp'" [alt]="testimonial.authorName" [rounded]="true" extraClasses="w-16 h-16 border-2 border-primary/50" />
                      <div>
        ...
        <section #animatedSection data-section-id="problem-solve" class="fade-in-section">
            <royal-code-ui-title [level]="TitleTypeEnum.H2" [text]="'cv.home.problemISolve.title' | translate" [center]="true" extraClasses="!mb-8" />
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-6xl mx-auto" style="grid-auto-rows: 1fr;">
                @for(problemKey of problemKeys; track problemKey) {
                    <royal-code-ui-card borderColor="primary" extraContentClasses="lift-card p-4">
      ...
      <article>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 max-w-7xl mx-auto">
          <div class="lg:col-span-1 space-y-4">
            <h4 class="font-bold text-lg text-center lg:text-left">{{ 'cv.skills.tShaped.deepSpecialization.title' | translate }}</h4>
 ...
 * @version 1.2.0 (Architectural Containment Fix)
 * @description
 *   Deze versie introduceert een cruciale 'overflow-x-hidden' op de root-container.
 *   Dit fungeert als een 'firewall' die rendering-conflicten en horizontale overflow,
 *   veroorzaakt door visueel complexe componenten, definitief voorkomt.
 */
import { ChangeDetectionStrategy, Component } from '@angular/core';
import { RouterModule } from '@angular/router';
...
@Component({
  ...
  standalone: true,
  imports: [RouterModule, CvHeaderComponent],
  template: `
 ...
 * @Version 6.0.0 (Clean Mobile UI: Hamburger-Only)
 * @Author Roy van de Wetering & Royal-Code MonorepoAppDevAI
 * @Date 2025-08-07
 * @Description Header component voor het CV. Deze definitieve versie verwijdert de mobiele
 *              bottom-bar voor een schonere interface, en centraliseert de mobiele
 *              navigatie-trigger in een enkele hamburger-knop linksboven.
 */
import { ChangeDetectionStrategy, Component, signal, Signal } from '@angular/core';
...
import { UiDesktopNavComponent, UiMobileNavModalComponent } from '@royal-code/ui/navigation';
...
<header class="sticky top-0 z-30 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div class="container-max px-4 md:px-6 h-16 flex items-center justify-between">
        <!-- LEFT SIDE: Mobile Hamburger or Desktop Logo -->
        <div class="flex items-center">
            <!-- Mobile Hamburger Button (Top-left) -->
            <button class="md:hidden p-2 -ml-2 rounded-md text-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                    (click)="openMobileMenu()"
                    [attr.aria-label]="'cv.navigation.openMenu' | translate">
                <royal-code-ui-icon [icon]="AppIcon.Menu" sizeVariant="md"></royal-code-ui-icon>
            </button>
        </div>

        <!-- CENTER: Desktop Navigatiemenu (hidden on mobile) -->
        <nav class="hidden md:flex flex-grow justify-center">
          <royal-code-ui-desktop-nav [menuItems]="desktopNavItems()" />
        </nav>

        <!-- RIGHT SIDE: Theme & Language Selectors -->
    ...
    <!-- Mobiele Navigatie (Modal/Side-drawer) -->
...
export class CvHeaderComponent {
  ...
  // Navigatie-items voor desktop (plat)
  readonly desktopNavItems: Signal<NavigationItem[]> = signal<NavigationItem[]>([
    { id: 'home-cv', labelKey: 'cv.navigation.home', route: '/', icon: AppIcon.Home },
    { id: 'projects-cv', labelKey: 'cv.navigation.projects', route: '/projects', icon: AppIcon.Package },
    { id: 'experience-cv', labelKey: 'cv.navigation.experience', route: '/werkervaring', icon: AppIcon.Briefcase },
    { id: 'skills-cv', labelKey: 'cv.navigation.skills', route: '/skills', icon: AppIcon.Gauge },
    { id: 'architecture-cv', labelKey: 'cv.navigation.architecture', route: '/architectuur', icon: AppIcon.Grid },
    { id: 'ai-workflow-cv', labelKey: 'cv.navigation.aiWorkflow', route: '/ai-workflow', icon: AppIcon.Bot },
    { id: 'about-me-cv', labelKey: 'cv.navigation.aboutMe', route: '/over-mij', icon: AppIcon.UserCircle },
    { id: 'contact-cv', labelKey: 'cv.navigation.contact', route: '/contact', icon: AppIcon.Mail },
  ]);

  // Navigatie-items voor mobiele modal (HIËRARCHISCH)
  readonly mobileModalRootItems: Signal<NavigationItem[]> = signal<NavigationItem[]>([
    {
      id: 'main-menu-root',
      labelKey: 'cv.navigation.menu.title',
      children: [
        { id: 'modal-home', labelKey: 'cv.navigation.home', route: '/', icon: AppIcon.Home },
        { id: 'modal-projects', labelKey: 'cv.navigation.projects', route: '/projects', icon: AppIcon.Package },
        { id: 'modal-experience', labelKey: 'cv.navigation.experience', route: '/werkervaring', icon: AppIcon.Briefcase },
        { id: 'modal-skills', labelKey: 'cv.navigation.skills', route: '/skills', icon: AppIcon.Gauge },
        { id: 'modal-architecture', labelKey: 'cv.navigation.architecture', route: '/architectuur', icon: AppIcon.Grid },
        { id: 'modal-ai-workflow', labelKey: 'cv.navigation.aiWorkflow', route: '/ai-workflow', icon: AppIcon.Bot },
        { id: 'modal-about-me', labelKey: 'cv.navigation.aboutMe', route: '/over-mij', icon: AppIcon.UserCircle },
        { id: 'modal-contact', labelKey: 'cv.navigation.contact', route: '/contact', icon: AppIcon.Mail },
      ]
    }
  ]);
  ...
  handleMobileNavigation(item: NavigationItem): void {
    if (item.route) {
      this.router.navigateByUrl(item.route as string);
    }
...
--- START OF FILE libs/core/routing/src/lib/services/breadcrumb.service.ts ---
...
import { TitleTypeEnum } from '@royal-code/ui/title';
 ...
 * @Version 3.0.0 (Fixed QueryParams Support)
 * @Author Royal-Code MonorepoAppDevAI & User
 * @Date 2025-09-07
 * @Description
 *   FIXED: Added support for queryParams and queryParamsHandling to enable
 *   proper navigation with URL parameters for category filtering.
 */
...
export class UiDesktopNavComponent {
  ...
  readonly linkClasses = computed(() => {
    ...
    if (this.isSubtle()) {
      return `${base} text-xs text-secondary hover:text-foreground`;
    }
    return `${base} text-sm text-foreground hover:text-primary`;
  });
}
--- END OF FILE ---
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, TranslateModule, UiIconComponent, UiNavigationCardComponent, UiButtonComponent, UiCategoryCardComponent],
 ...
 * @version 2.5.2 (Upgraded & Integrated with NavDisplayHintEnum)
 * @Author Royal-Code MonorepoAppDevAI & User
 * @Date 2025-08-31
 * @Description Renders a side drawer menu for mobile devices with smooth animations.
 *              This version is integrated into the CV app and resolves all browser-specific bugs,
 *              now correctly using `NavDisplayHintEnum`.
 */
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, UiIconComponent, TranslateModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    @if (isOpen()) {
      ...
      <div [@slideInRight]
          ...
          <div class="flex justify-between items-center mb-4 pb-3 border-b border-border flex-shrink-0">
              <button (click)="goBack()"
                      class="p-2 -ml-2 text-muted-foreground hover:text-foreground rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                      [attr.aria-label]="menuStack().length > 0 ? ('common.buttons.back' | translate) : ('common.buttons.close' | translate)">
                      <royal-code-ui-icon [icon]="AppIcon.ArrowLeft" sizeVariant="md"></royal-code-ui-icon>
              </button>
              ...
              <button (click)="requestClose()"
                      class="p-2 -mr-2 text-muted-foreground hover:text-foreground rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                  ...
                  @for (item of currentItems(); track trackItemId($index, item)) {
                      @if (item.displayHint?.includes(NavDisplayHintEnum.MobileModal)) {
                          ...
                          <button (click)="handleItemClick(item)"
                                  class="flex items-center p-3 space-x-3 rounded-md active:scale-[0.98] transition-all text-left w-full group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                                  [class.bg-primary]="isActive(item)"
                                  [class.text-primary-on]="isActive(item)"
                                  [class.hover:bg-hover]="!isActive(item)"
                                  [class.hover:text-foreground]="!isActive(item)">
                                  @if(item.icon; as iconName){
                                    <royal-code-ui-icon [icon]="iconName" sizeVariant="md" class="flex-shrink-0"
                                      [class.text-primary-on]="isActive(item)"
                                      [class.text-muted-foreground]="!isActive(item)"
                                      [class.group-hover:text-foreground]="!isActive(item)"/>
                                  } @else {
                                    <span class="w-5 h-5 mr-3 flex-shrink-0"></span>
                                  }
                              <span class="text-sm font-medium flex-grow truncate"
                                    [class.text-primary-on]="isActive(item)"
                              ...
                              @if (item.children && item.children.length > 0) {
                                <royal-code-ui-icon [icon]="AppIcon.ChevronRight" sizeVariant="sm" extraClass="text-muted-foreground ml-auto flex-shrink-0"/>
                              }
                          </button>
                      }
                  }
...
export class UiMobileNavModalComponent {
  ...
  isActive(item: NavigationItem): boolean {
    ...
    return this.router.isActive(item.route as string, {
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, TranslateModule, UiIconComponent, UiButtonComponent, UiImageComponent, UiListComponent, UiBadgeComponent, UiParagraphComponent],
  template: `
    <div
